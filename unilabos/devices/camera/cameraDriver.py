...
class CameraController:
    ...
    def __init__(
        ...
        try:
            ...
            # 避免初始化阶段崩溃，打印错误日志
            print(f"[CameraController] __init__ 中自动 start 失败: {e}", file=sys.stderr)
    ...
    def start(self, config: Optional[Dict[str, Any]] = None):
        ...
        # === start 时启动 FFmpeg 推流 ===
        print(
            "[CameraController] start() called, 启动 FFmpeg 推流到 SRS...",
            file=sys.stderr,
        )
        self._start_ffmpeg()
        # =================================================
        ...
        if self._ws and self._loop is not None:
            ...
            async def close_ws():
                try:
                    ...
                    print(
                        f"[CameraController] 关闭 WebSocket 时异常: {e}",
                        file=sys.stderr,
                    )
        ...
        while self._running:
            try:
                async with websockets.connect(self.signal_backend_url) as ws:
                    self._ws = ws
                    print(
                        f"[CameraController] WebSocket 已连接: {self.signal_backend_url}",
                        file=sys.stderr,
                    )
                ...
                if self._running:
                    print(
                        f"[CameraController] WebSocket 连接异常: {e}",
                        file=sys.stderr,
                    )
    ...
    async def _recv_loop(self):
        ...
        async for message in ws:
            try:
                ...
                print(
                    f"[CameraController] 收到非 JSON 消息: {message}",
                    file=sys.stderr,
                )
                continue
    ...
    async def _handle_message(self, data: Dict[str, Any]):
        ...
        if cmd == "start_stream":
            print("[CameraController] 收到 start_stream", file=sys.stderr)
            # start() 已经启动过 FFmpeg，这里多调一次会在 _start_ffmpeg 中被忽略
            self._start_ffmpeg()
            return

        if cmd == "stop_stream":
            print("[CameraController] 收到 stop_stream", file=sys.stderr)
            self._stop_ffmpeg()
            return

        if data.get("type") == "offer":
            print("[CameraController] 收到 WebRTC offer", file=sys.stderr)
    ...
    def _start_ffmpeg(self):
        ...
        if self._ffmpeg_process and self._ffmpeg_process.poll() is None:
            print(
                "[CameraController] FFmpeg 已在运行，跳过启动。",
                file=sys.stderr,
            )
            return
        ...
        print(
            f"[CameraController] 启动 FFmpeg: {' '.join(cmd)}",
            file=sys.stderr,
        )
        ...
        try:
            ...
            print(f"[CameraController] 启动 FFmpeg 失败: {e}", file=sys.stderr)
    ...
    def _stop_ffmpeg(self):
        ...
        """
        if self._ffmpeg_process and self._ffmpeg_process.poll() is None:
            print("[CameraController] 停止 FFmpeg...", file=sys.stderr)
    ...
    def _handle_webrtc_offer(self, offer_sdp: str) -> str:
        ...
        print(
            f"[CameraController] 向媒体服务器发送 Offer: url={self.webrtc_api}, "
            f"streamurl={self.webrtc_stream_url}",
            file=sys.stderr,
        )
        ...
        print(
            f"[CameraController] 媒体服务器响应: status={resp.status_code}, "
        ...
        if not answer_sdp:
            raise RuntimeError(f"媒体服务器返回的 SDP 为空: {data}")
        return answer_sdp